...
<c:set var="j" value="${j + 1}"/>						  
				...
				<div class="col-lg-9 col-md-9 col-sm-8" id="content">
						<c:forEach var="tmp" items="${productsList}" >
							  		<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">
							  			<form class="productForm form-inline" method="POST" id="addProd${tmp.produktid}" role="form">
							  				<div class="thumbnail">
									    		<a href="<c:url value="/prodspec?id=${tmp.produktid}" />">
									      			<img src="<c:url value="/images/${tmp.zdjecie}" />" title="Zobacz szczegóły" class="img-responsive"/>
									      		</a>
									      		<div class="caption">
										      		<p>${tmp.nazwa}</p>
										      		<p><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${tmp.cena}" />zł</p>
										      		 <c:if test="${pageContext.request.userPrincipal.name != null}">
										      		 	<div class="form-group">
										      		 		<div class="input-group">
											                    <input type="number" name="iloscprod" min="1" class="form-control">
											                    <span class="input-group-btn" title="Dodaj do koszyka">
															    	<button class="btn btn-default" type="button"><span data-product="${tmp.produktid}" class="addProductBtn glyphicon glyphicon-plus"></span></button>
															   	</span>							                   		
								                    		</div>
								                    	</div>	
								                    </c:if>
